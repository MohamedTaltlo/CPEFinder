...
func enableBroadcast(c *net.UDPConn) error {
	...
	if e != nil { return e }
	var se error
	e = rc.Control(func(fd uintptr) { se = syscall.SetsockoptInt(syscall.Handle(fd), syscall.SOL_SOCKET, syscall.SO_BROADCAST, 1) })
	if e != nil { return e }
	return se
}

func parseUBNT(data []byte) *Device {
	if len(data) < 4 || (data[0] != 1 && data[0] != 2) { return nil }
	d := &Device{}
	ips := []string{}
	for o := 4; o+3 <= len(data); {
		...
		case 0x02:
			for p := 0; p+10 <= len(v); p += 10 {
				m := macString(v[p:p+6]); ip := net.IP(v[p+6:p+10]).String()
				if d.MAC == "" { d.MAC = m }
				if validIPv4(ip) { ips = append(ips, ip) }
			}
		case 0x03: d.Firmware = cleanText(v)
		case 0x05: _ = v
		case 0x0B: d.Hostname = cleanText(v)
		case 0x0C: if d.Model == "" { d.Model = cleanText(v) }
		case 0x0D: d.SSID = cleanText(v)
		case 0x14, 0x15: if x := cleanText(v); x != "" { d.Model = x }
		case 0x1B: if d.Firmware == "" { d.Firmware = cleanText(v) }
		}
	}
	best := ""
	for _, ip := range ips { best = chooseIP(best, ip) }
	d.IP = best
...
func cleanText(b []byte) string { return strings.TrimSpace(strings.Trim(string(bytes.Trim(b, "\x00")), "\x00")) }
...
func (s *Scanner) genericUDP(stop <-chan struct{}) {
	for {
		...
		select { case <-stop: return; default: }
		s.pokeRound(stop)
		select { case <-stop: return; case <-time.After(2500 * time.Millisecond): }
	}
}
...
func (s *Scanner) ssdpRound(stop <-chan struct{}) {
	...
	for _, lip := range locals {
		...
		for {
			...
			if er != nil { break }
			txt := string(buf[:n]); mac := lookupARP(src.IP.String()); vendor := vendorFromMAC(mac)
			if vendor == "" {
				lower := strings.ToLower(txt)
				if strings.Contains(lower, "ubiquiti") || strings.Contains(lower, "airmax") || strings.Contains(lower, "wave") { vendor = "Ubiquiti" }
				if strings.Contains(lower, "mimosa") { vendor = "Mimosa" }
				if strings.Contains(lower, "cambium") || strings.Contains(lower, "epmp") { vendor = "Cambium" }
			}
			if vendor != "" { s.merge(Device{IP: src.IP.String(), MAC: mac, Vendor: vendor, Description: firstHeader(txt, "SERVER"), Method: "SSDP", LastSeen: time.Now()}) }
		}
		c.Close()
	}
}
...
func firstHeader(s, name string) string {
	for _, ln := range strings.Split(s, "\n") {
		...
		if len(p) == 2 && strings.EqualFold(strings.TrimSpace(p[0]), name) { return strings.TrimSpace(p[1]) }
	}
	return ""
}
...
type arpEntry struct { Index int; IP string; MAC string }
...
func getARPEntries() []arpEntry {
	...
	if r != 0 || len(buf) < 4 { return nil }
	n := *(*uint32)(unsafe.Pointer(&buf[0])); rowSize := uintptr(unsafe.Sizeof(mibIPNetRow{})); base := uintptr(4)
	entries := make([]arpEntry, 0, int(n))
	for i := uint32(0); i < n; i++ {
		off := base + uintptr(i)*rowSize
		if off+rowSize > uintptr(len(buf)) { break }
		row := (*mibIPNetRow)(unsafe.Pointer(&buf[off]))
		if row.PhysAddrLen < 6 || row.PhysAddrLen > 8 { continue }
...
func (s *Scanner) readARPCache() {
	for _, e := range getARPEntries() {
		if e.Index != s.adapter.Index { continue }
...
func lookupARP(ip string) string {
	for _, e := range getARPEntries() { if e.IP == ip { return e.MAC } }
	return ""
}
