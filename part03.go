...
var mimosaOUIs = makeSet([]string{"20:B5:C6", "84:9C:A4", "8C:B6:C5", "90:70:BF", "CC:54:FE", "D0:B0:DF"})

var cambiumOUIs = makeSet([]string{"00:04:56", "30:CB:C7", "58:C1:7A", "90:14:AF", "90:6D:62", "B4:A2:5C", "BC:A9:93", "BC:E6:7C", "FC:11:65"})
...
func (s *Scanner) merge(d Device) {
	...
	if d.Vendor == "" {
		...
		if strings.Contains(t, "ubiquiti") || strings.Contains(t, "airmax") || strings.Contains(t, "wave") {
			d.Vendor = "Ubiquiti"
		}
		if strings.Contains(t, "mimosa") {
			d.Vendor = "Mimosa"
		}
		if strings.Contains(t, "cambium") || strings.Contains(t, "epmp") || strings.Contains(t, "cnpilot") {
			d.Vendor = "Cambium"
		}
	}
	if d.MAC != "" {
	...
	if ok {
		...
		if d.MAC != "" && d.MAC != old.MAC {
			old.MAC = d.MAC
			changed = true
		}
		if d.Vendor != "" && d.Vendor != old.Vendor {
			old.Vendor = d.Vendor
			changed = true
		}
		if d.Model != "" && d.Model != old.Model {
			old.Model = d.Model
			changed = true
		}
		if d.Hostname != "" && d.Hostname != old.Hostname {
			old.Hostname = d.Hostname
			changed = true
		}
		if d.Firmware != "" && d.Firmware != old.Firmware {
			old.Firmware = d.Firmware
			changed = true
		}
		if d.SSID != "" && d.SSID != old.SSID {
			old.SSID = d.SSID
			changed = true
		}
		if d.Description != "" && d.Description != old.Description {
			old.Description = d.Description
			changed = true
		}
		if d.Method != "" && d.Method != old.Method && (old.Method == "" || methodScore(d.Method) >= methodScore(old.Method)) {
			old.Method = d.Method
			changed = true
		}
...
func enumerateAdapters() []Adapter {
	...
	adapterScore := func(a Adapter) int {
		...
		if strings.Contains(n, "ethernet") || strings.Contains(n, "lan") {
			sc += 40
		}
		if strings.Contains(n, "wi-fi") || strings.Contains(n, "wifi") || strings.Contains(n, "wlan") {
			sc += 10
		}
		if strings.Contains(n, "virtual") || strings.Contains(n, "vmware") || strings.Contains(n, "hyper-v") || strings.Contains(n, "tailscale") || strings.Contains(n, "wireguard") || strings.Contains(n, "bluetooth") || strings.Contains(n, "loopback") {
			sc -= 80
		}
		return sc
	}
